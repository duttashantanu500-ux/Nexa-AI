      ...
      <div className="mx-auto max-w-xl space-y-6 px-4 py-8">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <Field
              ...
              textarea
              placeholder="What the agent should do, step by step…"
            />
            <Field
              ...
              textarea
              placeholder="e.g. A list of 10 businesses with websites"
            />
            <Field
              label="Constraints (what not to do)"
              value={constraints}
              onChange={setConstraints}
              textarea
              placeholder="e.g. Do not invent companies; only use public sources"
            />
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-3">
            <p className="text-sm text-zinc-500">
              Only tools that are available can be selected.
            </p>
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-zinc-500">
                    {t.description} · {t.connector} · {t.permission}
                  </div>
                </div>
              </button>
            ))}
            <p className="text-xs text-zinc-400">
              Gmail, Slack, and other services will appear here when connected.
            </p>
          </div>
        )}
            ...
            {frequency !== "once" && (
              <>
                ...
                <p className="text-xs text-zinc-500">
                  Timezone: {timezone}. Scheduled background runs require the server
                  cron to be configured; you can always use Run now.
                </p>
              </>
            )}
          </div>
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <label className="flex items-center gap-2 text-sm">
              ...
              />
              Allow destructive actions (delete) — off by default
            </label>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4 text-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <Row label="Instructions" value={instructions || purpose} />
            <Row label="Tools" value={tools.join(", ")} />
            <Row label="Schedule" value={`${frequency}${frequency !== "once" ? ` at ${time} (${timezone})` : ""}`} />
            <Row label="Permissions" value={`${permMode}${allowDestructive ? ", destructive allowed" : ""}`} />
          </div>
        )}
